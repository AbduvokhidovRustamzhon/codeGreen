...
func (service *BurgersSvc) BurgersList(ctx context.Context) (list []models.Burger, err error) {
	...
	defer conn.Release()
	rows, err := conn.Query(ctx, "SELECT id, name, price, fileName, phone FROM burgers WHERE removed = FALSE")
	if err != nil {
		return nil, err // TODO: wrap to specific error
	}
	defer rows.Close()
	...
	for rows.Next() {
		item := models.Burger{}
		err := rows.Scan(&item.Id, &item.Name, &item.Price, &item.FileName, &item.Phone)
		if err != nil {
			return nil, err // TODO: wrap to specific error
		}
...
func (service *BurgersSvc) Save(ctx context.Context, model models.Burger) (err error) {
	...
	defer conn.Release()
	_, err = conn.Exec(context.Background(), "INSERT INTO burgers(name, price, fileName, phone) VALUES ($1, $2, $3, $4);", model.Name, model.Price, model.FileName, model.Phone)
	if err != nil {
